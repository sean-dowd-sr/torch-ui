...
import { createMemo, createSignal, createUniqueId, For, Show, splitProps, onCleanup } from 'solid-js'
  ...
  return (
    ...
    >
      ...
      <Show when={props.icon}>
        <span class="shrink-0 text-current opacity-70">{renderIcon(props.icon)}</span>
      </Show>
  ...
  return (
    ...
    >
      ...
      <Show when={props.opt.icon}>
        <span class="shrink-0 text-current opacity-70">{renderIcon(props.opt.icon)}</span>
      </Show>
    ...
    <div ref={local.ref} class={cn('w-full', local.class)}>
                ...
                <span class="flex items-center gap-2">
                  <Show when={itemProps.item.rawValue.icon}>
                    <span class="flex-shrink-0 text-ink-500">
                      {renderIcon(itemProps.item.rawValue.icon)}
                    </span>
                  </Show>
      ...
      >
        ...
        <Show when={!local.bare && local.error}>
          <p id={errorId()} class="mt-2 text-sm text-danger-600">
            {local.error}
          </p>
        </Show>
