...
function TreeView(props) {
  ...
  const icons = useIcons();
  const [internalSelected, setInternalSelected] = createSignal2(local.defaultSelected);
  ...
  const renderNodes = (nodes, level = 0) => <ul role={level === 0 ? "tree" : "group"} class="list-none">
    ...
    return <li
      role="treeitem"
      aria-expanded={hasChildren() ? expanded() ? "true" : "false" : void 0}
    ...
    >
      ...
      onKeyDown={(e) => {
        ...
        const treeBtns = () => root ? Array.from(
          root.querySelectorAll(
            "button[data-tree-item]:not(:disabled)"
          )
        ) : [];
        if (e.key === "ArrowDown") {
          e.preventDefault();
    ...
    >
      ...
      /* Node icon */
                ...
                <Show7 when={node.icon}>
                  <span
      class={cn(
        "flex mr-1.5 shrink-0 items-center justify-center [&>svg]:h-4 [&>svg]:w-4",
        selected() ? "text-primary-500" : "text-ink-500"
      )}
    >
                    {node.icon}
                  </span>
